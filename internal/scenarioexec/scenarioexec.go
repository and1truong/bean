...
func (w *walker) Manual(ctx context.Context, op Manual) (string, error) {
	...
	payload := fmt.Sprintf(`{"op":%q,"ref":%q,"name":%q,"url":%q,"text":%q,"secret":%q,"value":%q,"key":%q,"condition":%q,"as":%q,"attribute":%q,"ok":%t,"result":%s}`, op.Op, op.Ref, name, op.URL, op.Text, op.Secret, op.Value, op.Key, op.Condition, op.As, op.Attribute, err == nil, jsonString(w.scrub(outcome)))
