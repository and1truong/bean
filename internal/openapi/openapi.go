...
func Generate(a *appir.App) (json.RawMessage, error) {
	...
	for name, v := range a.Views {
		...
		paths["/api/views/"+name] = map[string]any{"get": map[string]any{"operationId": "view_" + name, "parameters": []any{map[string]any{"name": "limit", "in": "query", "schema": map[string]any{"type": "integer", "maximum": v.MaxLimit}}}, "responses": map[string]any{"200": map[string]any{"description": "View result", "content": map[string]any{"application/json": map[string]any{"schema": map[string]any{"type": "object", "properties": map[string]any{"data": map[string]any{"type": "array", "items": map[string]any{"type": "object", "properties": props}}}}}}}, "401": errorResponse(), "403": errorResponse()}}}
	}
	for name, a := range a.Actions {
		...
		paths["/api/actions/"+name] = map[string]any{"post": map[string]any{"operationId": "action_" + name, "security": []any{map[string]any{"cookieAuth": []any{}}}, "requestBody": map[string]any{"required": true, "content": map[string]any{"application/json": map[string]any{"schema": map[string]any{"type": "object", "properties": props, "required": required}}}}, "responses": map[string]any{"200": map[string]any{"description": "Action result"}, "400": errorResponse(), "401": errorResponse(), "403": errorResponse(), "409": errorResponse()}}}
	}
