...
func TestSequencePublicationSurvivesRestart(t *testing.T) {
	...
	if !exists || active.FormatVersion != appir.CurrentFormat || active.ReleaseID != published.ID || len(sequence.Frames) != 10 || len(active.Blocks["runtime_architecture"].Content) == 0 {
