...
test('scoped Book contents resolve, edit, filter, and clean up atomically',async({page,request,bean})=>{
  ...
  await expect(panelWorkspace.locator('nav[aria-label="Primary navigation"][data-variant="line"]')).toHaveCount(1)
  await expect(panelWorkspace.getByRole('navigation',{name:'Section navigation'})).not.toHaveAttribute('role','tablist')
  ...
  await expect(page.getByRole('link',{name:'Validate'})).toHaveAttribute('data-state','active')
