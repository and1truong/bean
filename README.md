...
Bean v0.6 alpha is a compiled application runtime designed for humans and AI agents to build operational web applications from high-level definitions. YAML or typed visual Studio definitions compile into a versioned immutable application, native SQLite or PostgreSQL tables, REST/OpenAPI operations, a metadata-driven administration console, and public React pages. Bean-owned UI surfaces use checked-in shadcn/ui components with shared design tokens. The frontend and SQLite driver ship in one executable.
