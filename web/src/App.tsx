...
function ViewBlockPage({name,block,display,displayName,displays={},filters={},pageFilters=noPageFilters,fieldTypes={},presentation,searchFields=[],formattedFields,fileFields,maxRows,path}:ViewBlockProps&{path:string}){
	...
	const[urlParams,setURLParams]=useSearchParams();const displayParameter=block+'.display';const activeDisplayName=urlParams.get(displayParameter)||displayName||'';display=displays[activeDisplayName]||display;const controls=useMemo(()=>(display?.Controls||[]).filter(control=>!pageFilters[control.Filter]),[display?.Controls,pageFilters]);const pageDisplay=display?.Type==='page'
...
function CalendarView({rows,presentation}:{rows:Row[];presentation:ViewPresentation}){return <ol className="grid gap-3 sm:grid-cols-2" data-testid="calendar-view">{[...rows].sort((a,b)=>String(a[presentation.TimeField||'']).localeCompare(String(b[presentation.TimeField||'']))).map(row=><li key={String(row.id)}><Card><CardHeader><CardTitle>{row[presentation.TitleField||'title']||row.id}</CardTitle><CardDescription>{formatDemoDate(row[presentation.TimeField||''])} – {formatDemoDate(row[presentation.EndField||''])}</CardDescription></CardHeader></Card></li>)}</ol>}
