...
function ViewBody({row,view,field,rich,file}:{row:Row;view:string;field:string;rich:boolean;file:boolean}){const selected=row[field];const value=String(selected??row.excerpt??row.description??'');if(file&&selected)return <Button variant="outline" asChild><a href={'/api/files/'+encodeURIComponent(String(selected))+'?view='+encodeURIComponent(view)}>Download attachment</a></Button>;return rich&&selected!==null&&selected!==undefined?<div className="rich-text" dangerouslySetInnerHTML={{__html:String(selected)}}/>:<p className="leading-7">{value}</p>}
