...
import {api,APIError,FormElement,Manifest,Node,PageFilter,Session,ViewDisplay,ViewDrill,ViewFilter,ViewPresentation} from './api'
...
const noPageFilters:Record<string,string>={}
...
type ViewBlockProps={name:string;block:string;display?:ViewDisplay;displayName?:string;displays?:Record<string,ViewDisplay>;filters?:Record<string,ViewFilter>;pageFilters?:Record<string,string>;fieldTypes?:Record<string,string>;presentation:ViewPresentation;searchFields?:string[];formattedFields:string[];fileFields:string[];maxRows?:number}
function ViewBlock(props:ViewBlockProps){
  const path=useLocation().pathname
  return <ViewBlockPage key={`${props.name}:${props.block}:${path}`} {...props} path={path}/>
}
function ViewBlockPage({name,block,display,displayName,displays={},filters={},pageFilters=noPageFilters,fieldTypes={},presentation,searchFields=[],formattedFields,fileFields,maxRows,path}:ViewBlockProps&{path:string}){
	const[cursors,setCursors]=useState<string[]>([''])
	const[search,setSearch]=useState('');const[submittedSearch,setSubmittedSearch]=useState('')
	const[urlParams,setURLParams]=useSearchParams();const pageFilterValues=useContext(PageFilterValues);const boundPageFilters=Object.fromEntries(Object.entries(pageFilters).map(([filter,pageFilter])=>[filter,pageFilterValues[pageFilter]??urlParams.get(pageFilter)??'']));const pageFilterSignature=JSON.stringify(Object.entries(boundPageFilters).sort());const cursorPageFilterSignature=useRef(pageFilterSignature);const cursor=cursorPageFilterSignature.current===pageFilterSignature?cursors[cursors.length-1]:'';useEffect(()=>{if(cursorPageFilterSignature.current===pageFilterSignature)return;cursorPageFilterSignature.current=pageFilterSignature;setCursors([''])},[pageFilterSignature])
	const displayParameter=block+'.display';const activeDisplayName=urlParams.get(displayParameter)||displayName||'';display=displays[activeDisplayName]||display;const controls=useMemo(()=>(display?.Controls||[]).filter(control=>!pageFilters[control.Filter]),[display?.Controls,pageFilters]);const pageDisplay=display?.Type==='page'
  ...
  const result=useQuery({queryKey:['public-view',request],queryFn:async()=>{const first=await api<{data:Row[];nextCursor:string}>(request);if(!structured)return first;const data=[...first.data];if(data.length>structuredLimit||(mode==='detail'&&first.nextCursor))throw new APIError(structuredLimitError);if(mode==='detail')return {data,nextCursor:''};let nextCursor=first.nextCursor;while(nextCursor&&data.length<200){const nextQuery=new URLSearchParams(query);nextQuery.set('cursor',nextCursor);nextQuery.set('limit',String(200-data.length));const next=await api<{data:Row[];nextCursor:string}>('/api/views/'+name+'?'+nextQuery);data.push(...next.data);if(data.length>200)throw new APIError(structuredLimitError);nextCursor=next.nextCursor}if(nextCursor)throw new APIError(structuredLimitError);return {data,nextCursor:''}}})
	...
	else if(mode==='table')content=<TableView rows={result.data.data} fields={renderer?.Fields||[]} fieldTypes={fieldTypes} display={display}/>
	...
	else if(mode==='metric')content=<MetricView row={result.data.data[0]} presentation={effectivePresentation} drill={display?.Drill} filters={Object.fromEntries(query)}/>
	else if(mode==='chart')content=<ChartView rows={result.data.data} presentation={effectivePresentation} drill={display?.Drill} filters={Object.fromEntries(query)}/>
	else if(mode==='calendar')content=<CalendarView rows={result.data.data} presentation={effectivePresentation}/>
	...
	return <div className="space-y-4">{pageDisplay?<PageHeader title={resolvedTitle||display?.Title?.Fallback||humanize(name)} description={display?.Description}/>:display&&resolvedTitle?<h2 className="font-heading text-2xl font-semibold">{resolvedTitle}</h2>:null}{Object.keys(displays).length>1?<Field id={'display-'+block} label="Display"><NativeSelect id={'display-'+block} value={activeDisplayName} onChange={event=>{const next=new URLSearchParams(urlParams);next.set(displayParameter,event.target.value);setCursors(['']);setURLParams(next,{replace:true})}}>{Object.keys(displays).sort().map(name=><NativeSelectOption key={name} value={name}>{humanize(name)}</NativeSelectOption>)}</NativeSelect></Field>:null}{controls.length?<form className="grid items-end gap-4 sm:grid-cols-2 lg:flex lg:flex-wrap" onSubmit={applyControls}>{controls.map(control=><ViewFilterControl key={control.Filter} scope={block} control={control} filter={filters[control.Filter]} value={controlValues[control.Filter]??''} onChange={value=>setControlValues(current=>({...current,[control.Filter]:value}))}/>)}<Button type="submit">Apply</Button></form>:null}{(searchFields.length||effectivePresentation.SearchFields?.length)?<form className="flex gap-2" role="search" onSubmit={event=>{event.preventDefault();setCursors(['']);setSubmittedSearch(search)}}><Input aria-label={'Search '+name.replaceAll('_',' ')} type="search" value={search} onChange={event=>setSearch(event.target.value)}/><Button type="submit">Search</Button></form>:null}{content}{mode==='table'&&display?.Pager?.Type!=='none'&&result.data&&<Pagination previousDisabled={cursors.length===1} nextDisabled={!result.data.nextCursor} previous={()=>setCursors(value=>value.slice(0,-1))} next={()=>setCursors(value=>[...value,result.data.nextCursor])}/>}</div>
}

function TableView({rows,fields,fieldTypes,display}:{rows:Row[];fields:NonNullable<ViewDisplay['Renderer']['Fields']>;fieldTypes:Record<string,string>;display?:ViewDisplay}){const selectable=Boolean(display?.Actions?.length)&&display?.Selection!=='none';const[selected,setSelected]=useState<string[]>([]);const toggle=(id:string,checked:boolean)=>setSelected(current=>checked?(display?.Selection==='single'?[id]:[...current.filter(value=>value!==id),id]):current.filter(value=>value!==id));return <div className="space-y-4"><Table><TableHeader><TableRow>{selectable&&<TableHead scope="col">Select</TableHead>}{fields.map(column=><TableHead scope="col" key={column.Field}>{column.Label||humanize(column.Field)}</TableHead>)}</TableRow></TableHeader><TableBody>{rows.map((row,index)=>{const id=String(row.id??index);return <TableRow key={id}>{selectable&&<TableCell><Checkbox aria-label={'Select '+String(row[fields[0]?.Field]??id)} checked={selected.includes(id)} onCheckedChange={checked=>toggle(id,Boolean(checked))}/></TableCell>}{fields.map(column=><TableCell key={column.Field}>{column.LinkRoute?<Link className="font-medium text-primary hover:underline" to={viewLink(column.LinkRoute,row)}>{displayValue(row[column.Field],fieldTypes[column.Field])}</Link>:displayValue(row[column.Field],fieldTypes[column.Field])}</TableCell>)}</TableRow>})}</TableBody></Table>{selectable&&selected.length>0?<RecordActions names={display?.Actions||[]} ids={selected} onDone={()=>setSelected([])}/>:null}</div>}
function RecordActions({names,ids,onDone}:{names:string[];ids:string[];onDone:()=>void}){const qc=useQueryClient();const manifest=useQuery({queryKey:['manifest'],queryFn:()=>api<Manifest>('/api/system/manifest')});const[name,setName]=useState(names[0]||'');const[values,setValues]=useState<Row>({});const[result,setResult]=useState('');const action=manifest.data?.actions?.[name];const run=useMutation({mutationFn:()=>runActionBatch(name,ids,values),onSuccess:output=>{const succeeded=output.results.filter(item=>item.ok).length;const failed=output.results.length-succeeded;setResult(`${succeeded} succeeded, ${failed} failed.`);void qc.invalidateQueries({queryKey:['public-view']});if(!failed)onDone()}});return <form className="space-y-3 rounded-lg border p-4" onSubmit={event=>{event.preventDefault();run.mutate()}}><div className="grid gap-3 sm:grid-cols-2"><Field id="record-action" label="Action"><NativeSelect id="record-action" value={name} onChange={event=>{setName(event.target.value);setValues({})}}>{names.map(value=><NativeSelectOption key={value} value={value}>{humanize(value)}</NativeSelectOption>)}</NativeSelect></Field>{action&&Object.entries(action.Input).filter(([field])=>field!=='id'&&!action.Derive?.[field]).map(([field,definition])=><RecordActionInput key={field} name={field} definition={definition} value={values[field]} onChange={value=>setValues(current=>({...current,[field]:value}))}/>)}</div><Button type="submit" disabled={run.isPending}>{run.isPending?'Running…':`Run for ${ids.length}`}</Button>{run.error&&<ErrorAlert error={run.error}/>} {result&&<StatusAlert>{result}</StatusAlert>}</form>}
function RecordActionInput({name,definition,value,onChange}:{name:string;definition:import('./api').Field;value:any;onChange:(value:any)=>void}){const id='record-action-'+name;const label=definition.Label||humanize(name);if(definition.Type==='enum')return <Field id={id} label={label}><NativeSelect id={id} required={definition.Required} value={String(value??'')} onChange={event=>onChange(event.target.value)}><NativeSelectOption value="">Select</NativeSelectOption>{definition.Options?.map(option=><NativeSelectOption value={option} key={option}>{humanize(option)}</NativeSelectOption>)}</NativeSelect></Field>;if(definition.Type==='boolean')return <div className="flex items-center gap-2"><Checkbox id={id} checked={Boolean(value)} onCheckedChange={checked=>onChange(Boolean(checked))}/><Label htmlFor={id}>{label}</Label></div>;const numeric=['integer','decimal','money'].includes(definition.Type);const type=numeric?'number':definition.Type==='date'?'date':definition.Type==='datetime'?'datetime-local':definition.Type==='email'?'email':'text';return <Field id={id} label={label}><Input id={id} type={type} required={definition.Required} value={String(value??'')} onChange={event=>onChange(numeric?(event.target.value===''?'':Number(event.target.value)):event.target.value)}/></Field>}
...
function drillLink(drill:ViewDrill|undefined,row:Row,filters:Record<string,string>){if(!drill?.Route)return'';const query=new URLSearchParams();for(const binding of drill.Bindings||[]){const value=binding.Source==='group'?row[binding.Name]:filters[binding.Name];if(value!==undefined&&value!==null&&value!=='')query.set(binding.Filter,String(value))}return drill.Route+(query.size?'?'+query:'')}
function MetricView({row,presentation,drill,filters}:{row:Row;presentation:ViewPresentation;drill?:ViewDrill;filters:Record<string,string>}){const content=<Card><CardHeader><CardDescription>{presentation.MetricLabel||humanize(presentation.MetricField||'metric')}</CardDescription><CardTitle><span className="text-4xl" data-testid="metric-value">{String(row[presentation.MetricField||'']??0)}</span></CardTitle></CardHeader></Card>;const href=drillLink(drill,row,filters);return href?<Link aria-label={'Open '+(presentation.MetricLabel||'metric')+' records'} to={href}>{content}</Link>:content}
function ChartView({rows,presentation,drill,filters}:{rows:Row[];presentation:ViewPresentation;drill?:ViewDrill;filters:Record<string,string>}){const values=rows.map(row=>Number(row[presentation.MetricField||'']??0));const maximum=Math.max(...values,1);return <div className="space-y-3" role="img" aria-label={presentation.MetricLabel||'Bar chart'} data-testid="bar-chart">{rows.map((row,index)=>{const label=String(row[presentation.GroupField||'']??'Unknown');const value=values[index];const bar=<div className="grid grid-cols-[minmax(6rem,1fr)_3fr_auto] items-center gap-3"><span>{label}</span><div className="h-5 rounded bg-muted"><div className="h-5 rounded bg-primary" style={{width:`${value/maximum*100}%`}}/></div><span aria-label={`${label}: ${value}`}>{value}</span></div>;const href=drillLink(drill,row,filters);return href?<Link key={label} aria-label={`Open ${label} records`} to={href}>{bar}</Link>:<div key={label}>{bar}</div>})}</div>}
