...
function ViewBlockPage({name,block,presentation,formattedFields,fileFields,path}:ViewBlockProps&{path:string}){
  ...
  if(presentation.Mode==='board')return <BoardView rows={result.data.data} presentation={presentation} refresh={()=>result.refetch()}/>
...
function ViewBody({row,field,rich,file}:{row:Row;field:string;rich:boolean;file:boolean}){const selected=row[field];const value=String(selected??row.excerpt??row.description??'');if(file&&selected)return <Button variant="outline" asChild><a href={'/api/files/'+encodeURIComponent(String(selected))}>Download attachment</a></Button>;return rich&&selected!==null&&selected!==undefined?<div className="rich-text" dangerouslySetInnerHTML={{__html:String(selected)}}/>:<p className="leading-7">{value}</p>}
...
function BoardView({rows,presentation,refresh}:{rows:Row[];presentation:ViewPresentation;refresh:()=>Promise<unknown>}){
  const move=useMutation({mutationFn:({id,status}:{id:string;status:string})=>api('/api/actions/'+presentation.MoveAction,{method:'POST',body:JSON.stringify({id,[presentation.GroupField||'status']:status})}),onSuccess:()=>void refresh()})
  const columns=presentation.Columns||[];const group=presentation.GroupField||'status';const title=presentation.TitleField||'title'
  return <div className="overflow-x-auto"><div className="grid min-w-[48rem] gap-4" style={{gridTemplateColumns:`repeat(${columns.length}, minmax(15rem, 1fr))`}}>{columns.map(column=><section className="rounded-xl bg-muted/60 p-3" key={column}><h3 className="mb-3 font-semibold">{humanize(column)}</h3><div className="space-y-3">{rows.filter(row=>row[group]===column).map(row=><Card key={row.id}><CardHeader><CardTitle>{presentation.LinkRoute?<Link className="hover:underline" to={viewLink(presentation.LinkRoute,row)}>{row[title]}</Link>:row[title]}</CardTitle>{presentation.MetaFields?.length?<CardDescription>{presentation.MetaFields.map(field=>String(row[field]??'')).filter(Boolean).join(' · ')}</CardDescription>:null}</CardHeader><CardContent className="space-y-3"><p>{String(row[presentation.BodyField||'description']??'')}</p><Field id={'move-'+row.id} label="Status"><NativeSelect id={'move-'+row.id} aria-label={'Status for '+row[title]} value={String(row[group])} disabled={move.isPending} onChange={event=>move.mutate({id:String(row.id),status:event.target.value})}>{columns.map(value=><NativeSelectOption key={value} value={value}>{humanize(value)}</NativeSelectOption>)}</NativeSelect></Field></CardContent></Card>)}{!rows.some(row=>row[group]===column)&&<p className="text-sm text-muted-foreground">No tasks</p>}</div></section>)}</div>{move.error&&<ErrorAlert error={move.error}/>}</div>
}
