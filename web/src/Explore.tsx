...
import {api,AdminManifest,Entity} from './api'
...
export function Explore(){
  ...
  const definitions=useQuery({queryKey:['definitions'],queryFn:()=>api<Definition[]>('/api/admin/definitions')})
  ...
  const preview=useMutation({mutationFn:()=>api<Preview>('/api/admin/explore/preview',{method:'POST',body:JSON.stringify({name:viewName,spec,search,filter:filterName&&filterValue?{[filterName]:filterValue}:{},limit:25})}),onSuccess:()=>{setSaved(false)}})
  const validate=useMutation({mutationFn:()=>api<Validation>('/api/admin/releases/validate',{method:'POST'})})
  const save=useMutation({mutationFn:()=>api('/api/admin/definitions',{method:'POST',body:JSON.stringify({apiVersion:'bean/v1alpha1',kind:'View',metadata:{namespace:'default',name:viewName},spec})}),onSuccess:()=>{setSaved(true);setReplace(false);validate.mutate();void qc.invalidateQueries({queryKey:['definitions']})}})
  const conflict=Boolean(definitions.data?.some(definition=>definition.kind==='View'&&definition.metadata.name===viewName))
  ...
  return <Page><PageHeader title="Explore" description="Build a typed View from an existing Entity, preview it with current Policy, then save an ordinary draft definition."/>
    ...
    <div className="mt-6 flex flex-wrap items-center gap-3"><Button data-testid="explore-preview" disabled={!viewName||(mode==='records'&&!fields.includes('id'))||(mode==='groups'&&!groupField)||!aggregateAlias||preview.isPending} onClick={()=>preview.mutate()}>{preview.isPending?'Previewing…':'Preview'}</Button>{conflict&&<Check id="explore-replace" label="Replace existing draft" checked={replace} onChange={setReplace}/>}<Button variant="outline" data-testid="explore-save" disabled={!preview.data||preview.isPending||save.isPending||(conflict&&!replace)} onClick={()=>save.mutate()}>{save.isPending?'Saving…':conflict?'Replace draft':'Save draft'}</Button></div>
    {preview.error&&<ErrorAlert error={preview.error}/>} {save.error&&<ErrorAlert error={save.error}/>} {validate.error&&<ErrorAlert error={validate.error}/>} {saved&&<StatusAlert>Saved View {viewName} to the deterministic Studio draft.</StatusAlert>}{validate.data&&<div className="mt-3 rounded-lg bg-muted p-3 text-sm" data-testid="explore-validation"><p>{validate.data.valid?'Draft validates.':'Draft has diagnostics.'} Semantic changes: {validate.data.changes?.length||0}.</p>{Boolean(validate.data.diagnostics?.length)&&<pre className="mt-2 overflow-auto text-xs">{JSON.stringify(validate.data.diagnostics,null,2)}</pre>}</div>}
    </SectionCard>
    <SectionCard title="Preview" description="Preview is ephemeral and executes through the same compiled View service as a published definition.">{preview.isPending?<LoadingState label="Running View preview…"/>:preview.data?<PreviewTable rows={preview.data.data} fields={previewFields(preview.data.data,mode,fields)}/>:<p className="text-muted-foreground">Configure the View and run a preview.</p>}</SectionCard>
  </Page>
}
