...
function AdminField({field,value,onChange,manifest,readonly=false,idPrefix='admin'}:{field:AdminFieldDefinition;value:any;onChange:(value:any)=>void;manifest:AdminManifest;readonly?:boolean;idPrefix?:string}){
  ...
  if(field.Type==='file')return <Field id={id} label={label}>{value&&<a className="text-primary underline" href={'/api/files/'+encodeURIComponent(String(value))}>Download current file</a>}<Input id={id} data-testid={'field-'+field.Name} type="file" required={field.Required&&!value} onChange={event=>onChange(event.target.files?.[0])}/></Field>
