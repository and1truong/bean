...
function RecordEditor({manifest,resource,initial,create}:{manifest:AdminManifest;resource:AdminResource;initial:Row;create:boolean}){
  ...
  const save=useMutation({mutationFn:()=>{const body=create?pick(values,fields):{id:initial.id,...changed(initial,values,fields)};return callAction<{data:Row}>(actionName,body)},onSuccess:result=>{qc.setQueryData(['admin-record',resource.Name,result.data.id],result);void qc.invalidateQueries({queryKey:['admin-resource',resource.Name]});nav(create?'/admin/'+resource.Name+'/'+result.data.id:'/admin/'+resource.Name)}})
  const remove=useMutation({mutationFn:()=>callAction(resource.DeleteAction,{id:initial.id}),onSuccess:()=>{void qc.invalidateQueries({queryKey:['admin-resource',resource.Name]});nav('/admin/'+resource.Name)}})
...
function AdminField({field,value,error,onChange,manifest,view,readonly=false,idPrefix='admin'}:{field:AdminFieldDefinition;value:any;error?:string;onChange:(value:any)=>void;manifest:AdminManifest;view?:string;readonly?:boolean;idPrefix?:string}){
  ...
  if(field.Type==='password'||field.Sensitive)return <Field id={id} label={label} error={error}><Input id={id} data-testid={'field-'+field.Name} type="password" required={field.Required} value={value??''} onChange={event=>onChange(event.target.value)}/></Field>
  if(field.Type==='relation')return <RelationControl field={field} value={value} error={error} onChange={onChange} manifest={manifest}/>
  if(field.Type==='enum')return <Field id={id} label={label} error={error}><NativeSelect id={id} data-testid={'field-'+field.Name} required={field.Required} value={value??''} onChange={event=>onChange(event.target.value)}><NativeSelectOption value="">Select…</NativeSelectOption>{field.Options?.map(option=><NativeSelectOption key={option}>{option}</NativeSelectOption>)}</NativeSelect></Field>
  if(field.Type==='boolean')return <div className="grid gap-2"><div className="flex items-center gap-2"><Checkbox id={id} data-testid={'field-'+field.Name} checked={Boolean(value)} onCheckedChange={checked=>onChange(Boolean(checked))}/><Label htmlFor={id}>{label}</Label></div>{error&&<p className="text-sm text-destructive" role="alert">{error}</p>}</div>
  if(field.Type==='text'||field.Type==='richtext')return <Field id={id} label={label} error={error}><Textarea id={id} data-testid={'field-'+field.Name} required={field.Required} value={value??''} onChange={event=>onChange(event.target.value)}/></Field>
  if(field.Type==='file')return <Field id={id} label={label} error={error}>{value&&view&&<a className="text-primary underline" href={'/api/files/'+encodeURIComponent(String(value))+'?view='+encodeURIComponent(view)}>Download current file</a>}<Input id={id} data-testid={'field-'+field.Name} type="file" required={field.Required&&!value} onChange={event=>onChange(event.target.files?.[0])}/></Field>
  ...
  return <Field id={id} label={label} error={error}><Input id={id} data-testid={'field-'+field.Name} type={type} required={field.Required} value={value??''} onChange={event=>onChange(numeric?(event.target.value===''?'':Number(event.target.value)):event.target.value)}/></Field>
}
