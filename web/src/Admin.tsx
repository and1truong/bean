...
function RecordEditor({manifest,resource,initial,create,contextMenus,contextual}:{manifest:AdminManifest;resource:AdminResource;initial:Row;create:boolean;contextMenus?:ContextMenu[];contextual?:ContextualCreate}){
  ...
  const controls=Object.fromEntries(fields.map(name=>{const field=findField(entity,name);const readonly=!create&&protectedFields.has(name);return [name,field?<AdminField key={name} field={field} value={readonly?initial[name]:values[name]} error={(save.error as APIError|undefined)?.fields?.[name]} readonly={readonly} manifest={manifest} view={resource.View} onChange={value=>setValues(current=>({...current,[name]:value}))}/>:null]}))
