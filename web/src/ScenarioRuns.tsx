...
export function ScenarioRunDetail(){
  ...
  const outcome=run.Status==='completed'?`${passed}/${nodes.length||steps.length} checks passed`
    :run.Status==='failed'&&failedStep?`Check "${failedStep.NodeID}" failed`
    :run.Status==='failed'?'Run failed'
    :live(run.Status)?`${run.Status} — ${passed} check${passed===1?'':'s'} done`
    :run.Status
  return <Page><PageHeader title={scenario?.title||`Run ${run.Scenario}`} description={<span data-testid="run-outcome">{outcome} · {duration(run.StartedAt,run.FinishedAt)}{route&&<> · {route}</>}</span>} context={<Link className="text-sm text-muted-foreground underline" to="/studio/runs">All runs</Link>} action={<div className="flex items-center gap-2"><StatusIndicator status={statusKind(run.Status)} label={run.Status}/>{run.Status==='running'&&<Button variant="outline" data-testid="pause-run" disabled={control.isPending} onClick={()=>control.mutate('pause')}>Pause</Button>}{run.Status==='paused'&&<Button data-testid="resume-run" disabled={control.isPending} onClick={()=>control.mutate('resume')}>Resume</Button>}{live(run.Status)&&<Button variant="destructive" data-testid="stop-run" disabled={control.isPending} onClick={()=>control.mutate('stop')}>Stop</Button>}{(run.Status==='failed'||run.Status==='cancelled'||run.Status==='completed')&&<Button variant="outline" data-testid="retry-run" disabled={retry.isPending} onClick={()=>retry.mutate()}>Retry run</Button>}{steps.length>0&&<Button variant="outline" data-testid="save-as-test" disabled={saveAsTest.isPending} onClick={()=>saveAsTest.mutate()}>Save as test</Button>}{run.Status==='failed'&&<Button variant="outline" data-testid="propose-repair" disabled={repair.isPending} onClick={()=>{setRepairDraft(null);repair.mutate(undefined,{onSuccess:setRepairDraft})}}>{repair.isPending?'Repairing…':'Propose repair'}</Button>}</div>}/>
