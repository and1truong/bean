...
export type ViewRenderer={Type?:string;TitleField?:string;BodyField?:string;LinkRoute?:string;LinkField?:string;EmptyState?:string;MetaFields?:string[];RichTextFields?:string[];GroupField?:string;OrderField?:string;ParentField?:string;MoveAction?:string;Columns?:string[];MetricField?:string;MetricLabel?:string;TimeField?:string;EndField?:string;SearchFields?:string[];Fields?:ViewColumn[]}
export type ViewControl={Filter:string;Label?:string;Widget?:string;Default?:any}
export type ViewDrill={View:string;Display:string;Route:string;Bindings?:Array<{Source:string;Name:string;Filter:string}>}
...
export async function apiResponse<T>(path:string,init?:RequestInit):Promise<{data:T;response:Response}>{const headers=new Headers(init?.headers);if(init?.body&&typeof init.body==='string')headers.set('Content-Type','application/json');const csrf=sessionStorage.getItem('bean_csrf');if(csrf)headers.set('X-CSRF-Token',csrf);const response=await fetch(path,{...init,headers});const body=await response.json().catch(()=>({}));if(!response.ok)throw new APIError(body?.error?.message||response.statusText,body?.error?.fields);return {data:body as T,response}}
export async function api<T>(path:string,init?:RequestInit):Promise<T>{return (await apiResponse<T>(path,init)).data}
