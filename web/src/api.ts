...
export type FieldLayout={Groups:Array<{Name:string;Label:string;Columns:1|2;Fields:Array<{Field:string;Span:'single'|'full'}>}>}
...
export async function apiResponse<T>(path:string,init?:RequestInit):Promise<{data:T;response:Response}>{const headers=new Headers(init?.headers);if(init?.body&&typeof init.body==='string')headers.set('Content-Type','application/json');const csrf=sessionStorage.getItem('bean_csrf');if(csrf)headers.set('X-CSRF-Token',csrf);const response=await fetch(path,{...init,headers});const body=await response.json().catch(()=>({}));if(!response.ok)throw new APIError(body?.error?.message||response.statusText,body?.error?.fields,response.status);return {data:body as T,response}}
