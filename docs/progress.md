...
Bean v0.13 Typed Extension Boundary is active from v0.12 squash merge `fbaa54a`. Milestones 0–1 are complete and milestone 2 is active. `Extension` is a registry-owned Definition in immutable `bean/appir/v5`, with canonical schema, capabilities, stable `BEAN-E2871` diagnostics, inspection, semantic diff, metadata-only publication, restart, and explicit v1–v4 compatibility. Its closed contract accepts one HTTP transport, network permission, external-write effect, none-or-bearer authentication, bounded timeout/fixed retry, required idempotency, after-commit transaction semantics, and retry-then-fail behavior. Endpoints require HTTPS except for loopback HTTP; typed fields exclude secrets, relations, files, and storage semantics.

The HTTP contract carries one stable invocation/idempotency identity across bounded at-least-once retry. Metadata declares closed permissions/effects, none-or-bearer authentication requirements, timeout, fixed retry, required idempotency, after-commit transaction semantics, and terminal failure behavior. Bearer material is host configuration and never AppIR or application metadata. Semantic TestSuites inject typed provider mocks into the same intent/delivery path, assert ordered interactions offline, and retain the existing isolated case bounds. Commerce will prove the generic slice with an order-notification Extension; arbitrary scripts, synchronous provider results, WASM/plugins, OAuth, new queue infrastructure, and provider-specific core behavior remain excluded.
