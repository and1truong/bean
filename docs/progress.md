...
The sealed internal capability-registry refactor is active from merge commit `b0eaaf5`. The behavior-preserving inventory found three real extension seams: Definition kinds repeated across compile/schema/inspection, Action steps repeated across compiler/runtime/DemoSeed, and Block types repeated across validation/render/component selection. Milestones 0–3 are complete; milestone 4 is active. The shared registry primitive is immutable after explicit construction, rejects blank or duplicate entries, and exposes deterministic copied names without a global mutation API. One Definition-kind registry now owns compilation, schema/capability vocabulary, compiled lookup and names, reference inspection, and diagnostic candidate context; its completeness is checked against the source envelope vocabulary.

The Action-step registry now owns the thirteen-step vocabulary, compiler requirements and allowed value fields, declared read/mutation/event/job effects, and runtime handler dispatch. Runtime and compiler parity tests fail if either side drifts, and DemoSeed derives its safe single-transition classification from the same effects instead of a private operation list. Focused Action, compiler, DemoSeed, and repository-wide Go tests pass without changing transaction, Policy, result binding, or View-read behavior.
