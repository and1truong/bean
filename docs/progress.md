...
Dynamic hierarchical Menu navigation is active. The first contract is frozen around global static Page/View targets, owner-scoped Entity-record placements, three parent-derived levels, deterministic weight ordering, typed route resolution, Action-atomic placement updates and cleanup, server Policy filtering, and responsive route navigation. AppIR v13 now stores bounded Menu owner/profile/static-target metadata and Entity navigation destinations with explicit v12 compatibility; compiler/schema/capabilities/inspection/diff coverage passes. Portable dynamic placement persistence and publication preflight are next; placements remain application data and the Book/Page acceptance slice will demonstrate two scoped Menu instances plus one record reused across both.
