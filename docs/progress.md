...
Bean v0.13 Typed Extension Boundary is active from v0.12 squash merge `fbaa54a`. Milestone 0 is complete and milestone 1 is active. The frozen slice adds one canonical Extension definition and one portable out-of-process HTTP transport. A transaction Action binds typed inputs and atomically persists an after-commit extension intent alongside domain mutation, audit, outbox, and Action idempotency state; provider output never controls the open transaction.
