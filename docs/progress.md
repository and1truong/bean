...
Bean v0.11 First-class Semantic Test Suites is active from v0.10 merge `38a6095` plus the maintained roadmap-definition commit `fe71804`. Milestones 0–4 are complete and milestone 5 is active. The frozen slice adds one canonical `TestSuite` Definition targeting Rule or Action identities, typed fixtures and explicit actor/tenant/fixed-time/deterministic-ID/seed context, result/error/mutation/event/audit assertions, bounded per-case isolation, and deterministic evidence through the existing `bean app test --json` contract. Direct Policy/Lifecycle targets, provider mocks, generated tests, standalone commands, and PostgreSQL-backed suite execution are out of scope.

TestSuite is registry-owned metadata with canonical schema, stable `BEAN-E2851` compiler diagnostics, capability limits, named inspection, typed references, redacted semantic diff, immutable `bean/appir/v4` storage, and restart compatibility. The compiler checks Rule and Action targets, machine-named cases, typed input/fixtures/context/assertions, fixture relations, closed error codes, and the 64-suite/128-case/1,024-fixture/1-MiB bounds while retaining source-order diagnostic locations before canonical case ordering.

The semantic runner gives every case a fresh published SQLite runtime and a five-second deadline. Rule cases call the production typed evaluator with only explicit context. Action cases call the production service with a fixed clock and explicit or seed-derived deterministic UUIDs, preserving Policy, guard, derive, invariant, transaction, audit, job, and outbox behavior. Fixture setup remains runner infrastructure; application assertions read through a runner-owned View identity, while event and audit evidence is confined to system tables. Case directories are removed after execution and replay tests prove stable results with no retained state.

`bean app test --json` retains all lifecycle smoke checks and adds ordered suite/case evidence tied to the candidate checksum. Assertion failures keep that evidence in unsuccessful envelopes and report `BEAN-T1001` with safe expected/actual digests. Maintained commerce, ATS, and booking metadata suites plus the generic Action contract cover Rule calculation, Action permission/guard behavior, fixed-time derivation, invariant rollback, mutation, event, and no-event behavior; seeded defect tests prove each contract detects regression. Focused compiler, AppIR, release/restart, Agent Protocol, CLI, runner, maintained-example, and complete Go tests pass.

Local terminal qualification passes on the v0.11 version cut: `make check` including race, black-box contracts, 30 React tests, and 14/14 Playwright journeys; `make test-crash`; `make test-postgres` including the PostgreSQL blog journey; and `make build`. The binary reports `bean 0.11.0-alpha`. PR review, CI, and merge remain.
