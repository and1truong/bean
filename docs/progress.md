...
Bean v0.12 Generated Semantic and Rule Tests is active from v0.11 merge `f224246`. Milestones 0–4 are complete and milestone 5 is active. The frozen slice generates only from canonical AppIR, explicit TestSuite expectations, and deterministic DemoSeed data. Runtime checks materialize as ordinary TestSuite definitions and retain explicit expectations as their oracle; structural checks report stable source-traced evidence. Generated Policy and Lifecycle negatives execute through Actions, CRUD writes use Actions, reads use Views, and browser journeys use the production HTTP surface. The generator does not infer business requirements or replace acceptance tests.

`generatedtest.Materialize` reserves the `generated_` identity prefix and emits canonical replay suites for every explicit Rule/Action suite, Policy-denial cases only when anonymous context is provably denied, invalid transition cases from Lifecycle enum/graph metadata, and create/update/delete smoke suites from relation-complete DemoSeed dependency fixtures. Every case is recompiled into immutable AppIR and runs in the v0.11 isolated production runner. Seeded calculation, guard, validation, fixed-context, Rule-limit, Policy, and Lifecycle mutations are caught by generated assertions; repeated complete `app test --json` output is byte-identical.

`bean.release.test` also reports ordered schema, Rule-bound/capability, Policy, Lifecycle, and route checks with stable source identities and evidence. After publication/restart it exercises eligible anonymous static Pages plus View display routes through the production HTTP handler. Commerce, ATS, and booking pass end-to-end generated checks; ATS additionally passes twelve DemoSeed CRUD suites with relation-aware create/delete fixtures. Existing isolation, bounds, diagnostic safety, and SQLite runner behavior remain authoritative. Provider mocks, direct Policy/Lifecycle interpreters, external effects, arbitrary scripts, PostgreSQL suite execution, and generated business expectations are out of scope. Focused generator, runner, Agent Protocol, CLI, and reference-application tests pass.

Local terminal qualification passes on the v0.12 version cut: `make check` including race, black-box contracts, 30 React tests, and 14/14 Playwright journeys; `make test-crash`; `make test-postgres` including Agent Protocol parity and the PostgreSQL blog journey; and `make build`. The binary reports `bean 0.12.0-alpha`. Direct `app test --json` runs pass for all ten maintained application manifests. PR review, CI, and merge remain.
