...
| Semantic content | AppIR v20 closed heading levels, paragraphs, unordered/ordered lists, quotes, code, callouts, compatible images, safe links, dividers, literal tables, media and single-choice quizzes without executable markup | complete | source/compiler/schema/AppIR/projection/React/browser contracts |
| Tabs composition | AppIR v20 named `tabs` Block with bounded direct ContentElement lists, whole-Block Policy, horizontal/vertical keyboard semantics, isolated lifecycle and complete print output | complete | compiler/schema/AppIR/Block/React/Playwright contracts |
| Explicit media loading | Compatible images plus transcript-first audio and validated-ID YouTube/video-playlist placeholders; no audio/provider source before Load and no persistence across hidden state | complete | URL corpus, projection, React and intercepted browser network contracts |
| Single-choice checks | Canonical `choices` with compiler-validated IDs/answer, native radios, explicit grading, retry, live feedback, instance isolation and no backend/storage path | complete | compiler/schema/AppIR/React/Playwright contracts |
...
| Presentation agent slice | Seventeen-frame, eight-chapter Bean introduction with vertical depth, semantic content/Tabs/media/choices, stable repair diagnostics, and a real grouped View/chart | complete | presentation source, agent rubric, restart, package, and browser evidence |
...
- Sequence v0.16 supports only the `presentation` profile and deterministic semantic density bounds. Static tables contain literal strings; quizzes and tabs have browser-local state only. Media has no proxy, download, transcoding, API lookup, analytics, transcript verification, or arbitrary provider/embed parameters. Freeform coordinates, arbitrary templates, animation, transitions, WYSIWYG editing, embedded agents, executable Markdown/HTML/CSS/JS/SVG, native PDF/PPTX, hosted sharing, asset caching, and pixel-perfect font/layout prediction are outside scope.
