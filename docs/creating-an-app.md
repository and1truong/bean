...
Use a `Sequence` when the application needs an ordered guided experience. Each named frame chooses a closed semantic layout and references an existing Panel; `direction: next` starts a horizontal chapter and `direction: down` adds detail within it. Omitted direction remains flat `next` navigation. Panels still place Blocks, and data-backed frames still use ordinary View Displays. Use `type: content` Blocks for bounded static content and `type: tabs` Blocks for bounded composition; see [Content Blocks](content-blocks.md) for the vocabulary, YAML, bounds, safe links, and media rules. Run `bean capabilities --json` for the exact contract, and see `examples/presentation` for the seventeen-frame Bean introduction. Do not embed HTML, CSS, JavaScript, SVG, or private query logic in presentation metadata.
